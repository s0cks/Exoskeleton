...
import exoskeleton.client.overlay.AssassinOverlay
...
object AssassinHandler{
  private val overlay = new AssassinOverlay;
  ...
  def render(e: RenderWorldLastEvent): Unit ={
    ...
    if(DataManager.get(player).infiltratorEnabled() && ArmorHelper.hasExoHelm(player)){
      overlay.draw(mc);
    }
  }
  ...
  def doXRay(ev: RenderWorldLastEvent): Unit ={
    ...
    if(DataManager.get(player).infiltratorEnabled() && ArmorHelper.hasExoHelm(player)){
      ...
      for(e: Entity <- entities.asInstanceOf[util.List[Entity]]){
        if(e.isInstanceOf[EntityLiving]){
          GL11.glPushMatrix();
