...
class AssassinOverlay
extends IOverlay{
  private var assassin: Integer = null;

  override def draw(minecraft: Minecraft): Unit ={
    if(this.assassin == null){
      this.assassin = GL11.glGenLists(2);
      GL11.glNewList(assassin, GL11.GL_COMPILE);
      ...
      tess.setColorRGBA(3, 57, 108, 100);
