...
object KeyHandler{
  ...
  private val key_backtrack = new KeyBinding("key.backtrack.desc", Keyboard.KEY_T, "key.exoskeleton.category");
  private val key_autosmelt = new KeyBinding("key.autosmelt.desc", Keyboard.KEY_G, "key.exoskeleton.category");
  private val key_vision = new KeyBinding("key.vision.desc", Keyboard.KEY_V, "key.exoskeleton.category");
  ...
  ClientRegistry.registerKeyBinding(this.key_vision);
  ...
  def onTick(e: PlayerTickEvent): Unit ={
    ...
    if(e.phase == Phase.START){
      ...
      if(this.key_vision.isPressed && FMLClientHandler.instance().getClient.inGameHasFocus){
        if(ArmorHelper.thermal(e.player)){
